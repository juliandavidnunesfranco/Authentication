...
app.use(morgan('dev'));
app.use(express.urlencoded({ extended: true }));  ///------>>>>>lee los html

// El orden es importante, el cookieparser debe estar antes de la utilización del session
app.use(cookieparser());   ///debe ir primero que el session
...
app.post('/register', redirectHome, (req, res) => {
  ...
  if(name && email && password) {
    const exists = users.some(user => user.email === email);  ///---->>>>cambiar
    if(!exists) {
      const user = {
        id: users.length + 1,
        name,
